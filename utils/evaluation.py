...
def bacc_score(y_pred, y_true):
    return metrics.balanced_accuracy_score(y_pred=y_pred, y_true=y_true)
...
def kappa(y_pred, y_true):
    return metrics.cohen_kappa_score(y2=y_pred, y1=y_true)
...
def aupr_score(y_pred, y_true):
    ...
    Returns:
        [type]: [description]
    """
    precision, recall, _ = metrics.precision_recall_curve(y_true=y_true, probas_pred=y_pred)
    return metrics.auc(x=recall, y=precision)